...
export const PaymentDialog: React.FC<Props> = ({ payout, open, onOpenChange }) => {
  ...
  const remaining = Math.max(Number(payout.net_payable) - Number(payout.paid_amount), 0);
  const [amount, setAmount] = useState<string>(String(remaining.toFixed(0)));
  ...
  const submit = async (closePartial: boolean) => {
    if (amt <= 0) { toast({ title: 'Enter amount', variant: 'destructive' }); return; }
    setBusy(true);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const { data, error } = await supabase.functions.invoke('record-payment-event', {
        body: { payout_id: payout.id, amount: amt, method, reference: reference || null, paid_on: paidOn, note: note || null, close_partial: closePartial },
        headers: session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : undefined,
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);
      toast({ title: 'Payment recorded', description: `${inr(amt)} via ${method}` });
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 text-sm">
          <div className="grid grid-cols-3 gap-2">
            <Stat label="Bill" value={inr(Number(payout.net_payable))} />
            <Stat label="Paid" value={inr(Number(payout.paid_amount))} />
            <Stat label="Remaining" value={inr(remaining)} highlight />
          </div>

          <div>
            <Label className="text-xs">Amount paying</Label>
            <div className="flex gap-2">
              <Input inputMode="numeric" value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ''))} className="text-base" />
              <Button type="button" size="sm" variant="outline" onClick={() => setAmount(String(remaining.toFixed(0)))}>Full</Button>
            </div>
          </div>
          ...
          {isPartial && (
            <div className="bg-amber-50 border border-amber-200 rounded p-2 text-xs">
              <strong>Partial payment.</strong> {inr(remaining - amt)} रहेगा। If you close this bill, the remaining amount will be added to next cycle's bill as carry-forward.
            </div>
          )}
        </div>
