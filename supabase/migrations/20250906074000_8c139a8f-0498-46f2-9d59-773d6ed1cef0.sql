...
CREATE INDEX IF NOT EXISTS idx_ai_records_pd_date ON public.ai_records USING btree (pd_date);

-- Optimize cows table for common queries
CREATE INDEX IF NOT EXISTS idx_cows_status ON public.cows USING btree (status);
CREATE INDEX IF NOT EXISTS idx_cows_milking_move ON public.cows USING btree (needs_milking_move, needs_milking_move_at);
...
CREATE INDEX IF NOT EXISTS idx_vaccination_records_next_due_date ON public.vaccination_records USING btree (next_due_date);